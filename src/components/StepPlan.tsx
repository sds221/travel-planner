...
function DayCard({ day }: { day: ItineraryDay }) {
  ...
  return (
    <section className="panel overflow-hidden">
      <header className="border-b border-border bg-secondary/40 p-4">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            {day.theme && (
              <p className="mt-0.5 text-xs leading-snug text-foreground">{day.theme}</p>
            )}
          </div>
        </div>
        ...
        {day.tip && (
          <p
            className="mt-2.5 rounded-lg border-l-2 border-foreground/50 bg-secondary/40
      ...
      <div className="p-4">
      {day.items.length === 0 ? (
        <p className="text-xs text-muted-foreground">这天空着，可以自由活动。</p>
      ) : (
